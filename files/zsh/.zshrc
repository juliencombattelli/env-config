# User configuration that must be done before sourcing oh-my-zsh
...
# User configuration that can be done after sourcing oh-my-zsh

# Disable auto cd feature as I never use it anyway
unsetopt autocd

# Source any .sh and .zsh script fro env-config
for f in /$HOME/.config/env-config/files/scripts/*; do
    if [[ "$f" =~ ^.*\\.(z)?sh$ ]]; then
        source "$f"
    fi
done
