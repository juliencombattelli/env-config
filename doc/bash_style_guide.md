...
Inspired by YSAP's Bash Style Guide.
Checkout the original version using `curl style.ysap.sh/md`.

This guide outlines how to write bash scripts with a style that makes them safe
and predictable.  This guide is the working document for how I approach bash
scripting when it comes to style, design, and best-practices.
...
Spaces (4 spaces per level of indentation).
...
Using the dedicated `function` keyword makes the declaration stands out more and
...
All variables created in a function should be made local with the `local`
or `declare` commands.

```bash
# wrong
function foo {
    i=foo # this is a global, wrong depending on intent
}
...
# right
function foo {
    declare i=foo # implicitly local, useful to declare arrays
}

# wrong
function foo {
    declare -g i=foo # this is a global, wrong depending on intent
}
...
Document functions as much as possible, especially their side-effects on global
variables and file descriptors.

```bash
# right
function foo {
    # Perform fooing, reading the first argument and setting the global GLOBAL_I
    # variable
    local i="$1"
    GLOBAL_I="$i"
}
...
Use the following snippet to robustly know the path of the current executable.
It resolves symlinks, and returns the current working directory if the script is
read from stdin.

```bash
function __dirname {
    # From https://stackoverflow.com/a/246128/13195557
